...
import com.canopus.Vmusic.data.model.ChannelSearchResult
import com.canopus.Vmusic.data.model.HolodexChannelMin
import com.canopus.Vmusic.data.model.HolodexVideoItem
import com.canopus.Vmusic.data.model.discovery.ChannelDetails
import com.canopus.Vmusic.data.store.UnifiedStoreFactory
import com.canopus.Vmusic.di.DefaultDispatcher
import com.canopus.Vmusic.playback.util.formatDurationSeconds
import com.canopus.Vmusic.util.IdUtil
...
class ChannelRepository @Inject constructor(
    ...
    fun getChannel(
        channelId: String,
        isExternal: Boolean
    ): Flow<StoreReadResponse<UnifiedDisplayItem>> {
        return store.stream(
            StoreReadRequest.cached(
                key = ChannelKey(channelId, isExternal),
                refresh = false
            )
        )
    }
    ...
    suspend fun getExternalChannelVideos(channelId: String): List<UnifiedDisplayItem> =
        withContext(defaultDispatcher) {
            try {
                val ytService = NewPipe.getService(ServiceList.YouTube.serviceId)
