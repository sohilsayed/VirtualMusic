...
class UnifiedVideoRepository @Inject constructor(
    ...
    fun getFavorites(): Flow<List<UnifiedDisplayItem>> {
        return unifiedDao.getOptimizedFavoritesFeed()
            .map { list -> list.map { it.toUnifiedDisplayItem() } }
    }

    fun getDownloads(): Flow<List<UnifiedDisplayItem>> {
        return unifiedDao.getOptimizedDownloadsFeed()
            .map { list -> list.map { it.toUnifiedDisplayItem() } }
    }

    fun getHistory(): Flow<List<UnifiedDisplayItem>> {
        return unifiedDao.getOptimizedHistoryFeed()
            .map { list -> list.map { it.toUnifiedDisplayItem() } }
    }

    fun getFavoriteChannels(): Flow<List<UnifiedDisplayItem>> {
        return unifiedDao.getFavoriteChannels()
            .map { list -> list.map { it.toUnifiedDisplayItem() } }
    }
    ...
    fun observeDownloadedIds(): Flow<Set<String>> {
        return unifiedDao.getDownloadsFeed().map { list -> list.map { it.metadata.id }.toSet() }
            .distinctUntilChanged()
    }

    suspend fun getDownloadedIdsSnapshot(): Set<String> {
        return unifiedDao.getAllDownloadsOneShot().filter { it.downloadStatus == "COMPLETED" }
            .map { it.itemId }.toSet()
    }
