...
import com.canopus.Vmusic.background.M4AExportWorker
import com.canopus.Vmusic.data.db.DownloadStatus
import com.canopus.Vmusic.data.db.UnifiedDao
import com.canopus.Vmusic.data.db.UnifiedMetadataEntity
import com.canopus.Vmusic.data.db.UserInteractionEntity
import com.canopus.Vmusic.data.model.HolodexChannelMin
import com.canopus.Vmusic.data.model.HolodexSong
import com.canopus.Vmusic.data.model.HolodexVideoItem
import com.canopus.Vmusic.di.ApplicationScope
import com.canopus.Vmusic.di.DownloadCache
import com.canopus.Vmusic.di.UpstreamDataSource
import com.canopus.Vmusic.playback.data.source.StreamResolutionCoordinator
import com.canopus.Vmusic.service.HolodexDownloadService
...
class DownloadRepositoryImpl @Inject constructor(
    ...
    private val _downloadCompletedEvents =
        MutableSharedFlow<DownloadRepository.DownloadCompletedEvent>()
        ...
        applicationScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val interaction = UserInteractionEntity(
                    ...
                    downloadProgress = 0,
                    downloadFileName = "${displayTitle.take(50)}.m4a"
                )
                unifiedDao.upsertInteraction(interaction)

                // 2. Resolve Stream URL
                val streamUrl =
                    streamResolutionCoordinator.getCachedUrl(video.id) ?: withTimeout(30_000) {
    ...
    override suspend fun retryExport(itemId: String) {
        ...
        val downloadInt =
            projection.interactions.find { it.interactionType == "DOWNLOAD" } ?: return
        ...
        val exportRequest =
            OneTimeWorkRequestBuilder<M4AExportWorker>().setInputData(workData).build()
    ...
    override suspend fun reconcileAllDownloads() {
        withContext(Dispatchers.IO) {
            ...
            val actuallyRunning =
                media3DownloadManager.currentDownloads.map { it.request.id }.toSet()
