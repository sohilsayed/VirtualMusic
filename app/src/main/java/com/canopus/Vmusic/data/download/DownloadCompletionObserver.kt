            ...
            when (download.state) {
                Download.STATE_COMPLETED -> {
                    ...
                    val workData = Data.Builder()
                        ...
                        .putString(
                            M4AExportWorker.KEY_ORIGINAL_URI,
                        ...
                        .putString(
                            M4AExportWorker.KEY_ALBUM_NAME,
                            projection.metadata.title
                        ) // Or parent title if available
                        .putString(
                            M4AExportWorker.KEY_ARTWORK_URI,
                            projection.metadata.specificArtUrl
                        )
                        .putLong(M4AExportWorker.KEY_CLIP_START_MS, clipStartMs)
                        .putLong(M4AExportWorker.KEY_CLIP_END_MS, clipEndMs)
                        .apply {
                            currentEntry.downloadTrackNum?.let {
                                putInt(
                                    M4AExportWorker.KEY_TRACK_NUMBER,
                                    it
                                )
                            }
                        }
