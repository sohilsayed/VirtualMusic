package com.canopus.Vmusic.playback.data.source
...
class StreamResolutionCoordinator(
    ...
    private data class ResolvedStreamCacheEntry(
    ...
    private suspend fun resolveSingleStreamInternal(
        ...
        val downloadInteraction =
            prewarmedDownloads?.get(item.id) ?: unifiedDao.getDownloadInteraction(item.id)
    ...
    private fun putStreamInCache(videoId: String, streamDetails: StreamDetails) {
        streamUrlCache.put(
            videoId,
            ResolvedStreamCacheEntry(streamDetails, System.currentTimeMillis())
        )
    }
