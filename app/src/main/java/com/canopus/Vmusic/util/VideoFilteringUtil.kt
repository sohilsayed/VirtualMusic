...
object VideoFilteringUtil {
    ...
    fun isMusicContent(video: HolodexVideoItem): Boolean {
        ...
        Timber.d(
            "isMusicContent Checking: ID=${videoLogId}, Type=${video.type}, Topic=${video.topicId}, Songcount=${video.songcount ?: 0}, Chan='${
                video.channel.name.take(
                    20
                )
            }'"
        )
        ...
        val potentiallyMusicRelatedTopics =
            setOf("3D_Stream", "FreeChat", "雑談", "misc", "unknown", null, "")
