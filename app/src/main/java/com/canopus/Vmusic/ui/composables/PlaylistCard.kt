...
fun PlaylistCard(
    ...
    val (type, displayTitle, displayDescription) = remember(playlist) {
        val playlistType =
            if (playlist.id.startsWith(":")) playlist.id.substringBefore('[') else "ugp"
        val title = PlaylistFormatter.getDisplayTitle(
            playlist,
            context
        ) { en, jp -> jp?.takeIf { it.isNotBlank() } ?: en }
        val description = PlaylistFormatter.getDisplayDescription(
            playlist,
            context
        ) { en, jp -> jp?.takeIf { it.isNotBlank() } ?: en }
