...
import com.canopus.Vmusic.ui.composables.HeroCarousel
...
private fun ShelfItemContent(
    ...
    when (item) {
        ...
        is SingingStreamShelfItem -> {
            val displayShell =
                item.video.toUnifiedDisplayItem(isLiked = false, downloadedSegmentIds = emptySet())
            UnifiedGridItem(
                item = displayShell,
                onClick = {
                    navController.navigate(AppDestinations.videoDetailRoute(item.video.id))
