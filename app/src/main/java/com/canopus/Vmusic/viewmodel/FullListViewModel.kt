...
import com.canopus.Vmusic.viewmodel.mappers.toVideoShell // <--- ADDED IMPORT
        ...
        result.onSuccess { response ->
            val newItems: List<Any> = when (response) {
                is List<*> -> {
                    ...
                    (response as List<com.canopus.Vmusic.data.model.discovery.MusicdexSong>).map { song ->
                ...
                is com.canopus.Vmusic.data.api.PlaylistListResponse -> response.items
                is com.canopus.Vmusic.data.api.PaginatedChannelsResponse -> response.items.map { it.toDiscoveryChannel() }
                else -> emptyList()
            }
...
private fun com.canopus.Vmusic.data.model.discovery.ChannelDetails.toDiscoveryChannel(): DiscoveryChannel {
